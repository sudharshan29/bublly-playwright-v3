...
export class InboxDataHelper {
  ...
  async createOwnedConversation(label: string): Promise<{ id: string; subject: string }> {
    ...
    // Poll for the new ticket — QA server ingestion can take 5-45s; up to 300s when widget server is cold.
    // Poll every 2s for up to 300s before giving up.
    ...
    if (!newTicket?.id) throw new Error(`Widget message sent but new ticket not found in Open inbox after 300s.`);
